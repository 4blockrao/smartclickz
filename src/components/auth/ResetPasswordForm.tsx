...
const ResetPasswordForm = ({ onPasswordSet, onBack }: ResetPasswordFormProps) => {
    ...
    <div className="min-h-[100vh] flex flex-col items-center justify-center bg-gradient-to-br from-[#9b87f5]/60 via-[#f1f0fb]/95 to-[#e9e4fd]/100 relative pb-8">
      ...
      <div className="w-full max-w-md px-6">
        <div className="mb-5 text-center">
          <h2 className="text-2xl sm:text-3xl font-bold text-[#7e69ab] drop-shadow">Set New Password</h2>
          <p className="mt-1 text-base text-gray-700 font-semibold">
            Choose a strong new password to secure your account
          </p>
        </div>
        <div className="p-7 sm:p-10 rounded-2xl shadow-2xl bg-white/90 backdrop-blur-lg border border-[#e9e4fd] flex flex-col gap-2">
          ...
          {onBack && (
            <button
              type="button"
              onClick={onBack}
              className="mt-3 text-sm text-[#7e69ab] hover:underline text-center w-full"
            >
              Back to login
            </button>
          )}
